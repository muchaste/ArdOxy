...
char fishID[8][6] = {"ID1", "ID2", "ID3", "ID4", "ID5", "ID6", "ID7", "ID8"};           // IDs assigned to the channels in the order of the channelArray
long interval = 30 * 1000UL;                                                            // measurement and control interval in second
double airSatThreshold[8] = {80.0, 100.0, 100.0, 100.0, 100.0, 100.0, 100.0, 100.0};    // air saturation threshold including first decimal                            
...
double Ki[channelNumber] = {0, 0, 0, 0, 0, 0, 0, 0};            // coefficient for integrative control
double Kd[channelNumber] = {0, 0, 0, 0, 0, 0, 0, 0};            // coefficient for differential control
...
//# Oxygen optode #
char toggleMeasString[6] = "SEQ 1";           // template for measurement commmand that is sent to sensor during void toggleMeasurement()
...
double airSatFloat[channelNumber];            // array for floating point value of air saturation for logging, display etc.
char airSatDisplay;                           // string template for serial display of measurements

//# Relay #
long int relayArray[4][channelNumber];        // array with relay pin and assigned output values
...
//### Sensor specific commands to toggle measurement and readout of DO values ###
...
void recvWithEndMarker() {      //receives serial data into array until endmarker is received
  ...
  char rc;
  if (activeSensor == 1) {
    while (Serial1.available() > 0 && newData == false && emptyBuffer == true) {
      rc = Serial1.read();
      if (rc != endMarker) {
...
int extractAirSat() {            
  ...
  len = len - 10;                                 // subtract to length of DO measurement digits
  if (len == 0) {
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Strlen = 0 :(");
    lcd.setCursor(0, 1);
    lcd.print("Sensor connect?");
    while (1);  
  }
  for (int i = 0; i < len; i++) {        // store the last digits of string in new object airSatStr, starting at and including position 10
    airSatStr[i] = receivedChars[(10 + i)];
  }
  airSatReading = atol(airSatStr);
  return airSatReading;
}

//### Send air saturation readings to serial monitor and LCD (NECESSARY, sets newData to false) ###
void showNewData() {
  if (newData == true && emptyBuffer == true) {
    ...
    lcd.print(int(airSatFloat[channelNumber - 1]));
    lcd.print(" ");
    newData = false;
  }
}
...
void toggleRelay() {
  ...
  for (int i = 0; i < channelNumber; i++) {
    ...
    relayArray[2][i] = Output[i]*200;                   //PID computes an output between 0 and 50, the multiplicator makes sure that the relay operation time is at least 200ms
    relayArray[3][i] = airSatFloat[i];
  }

  long int temp[4];
  for (int i = 0; i < channelNumber - 1; i++) {         // sort 2D-array with lowest difference first
...
//# Create logfile #
  ...
  if (logfile) {
    ...
    for (int i = 0; i < (channelNumber - 1); i++) {
      logfile.print(airSatThreshold[i] / 10);
      logfile.print(";");
    }
    logfile.println(airSatThreshold[channelNumber - 1] / 10);
...
void loop() {
  ...
  for (int i = 0; i < channelNumber; i++) {                   // loop that iterates through every channel
    ...
    sprintf(toggleMeasString, "SEQ %d", activeChannel);       // adjust channel in measurement command
    sprintf(toggleReadString, "REA %d 3 4", activeChannel);   // adjust channel in readout command
    for (int j = 0; j < samples; j++) {                       // start oversampling loop per channel
  ...
  if (lowDO == false){ 
    ...
    if (elapsed <= interval) {
      delay(interval - elapsed);                                  // delay so that loop duration equals measurement interval
    }
    else {                                                      // halt if loop takes longer than measurement interval
      Serial.println("Program halt: Measurement interval too short, set new interval!");
      Serial.print("Minimum duration [s]: ");
    ...
    Serial.println("Program halt: low DO!");
    lcd.clear();
    lcd.print("low DO!");
    while (1);
  }
